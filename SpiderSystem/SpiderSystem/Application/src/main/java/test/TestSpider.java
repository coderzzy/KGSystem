...
public class TestSpider {
	...
	//爬取《南农新闻网》的所有信息，并将信息输出到文本文件！
	
	public static void main(String[] args){
		Spider.build()
        .setProcessor(myPageProcessor)
        .setSaver(mySaver)
        .addUrlSeed("http://news.njau.edu.cn")
        .addRegexRule("http://news.njau.edu.cn/.*htm") //只爬取新闻类的页面
        .run();
	}
    ...
    static PageProcessor myPageProcessor = new PageProcessor() {
        ...
        public void process(Page page) {
            ...
            //如果不匹配，则不进行解析！
            if (!Pattern.matches("http://news.njau.edu.cn/.*htm", page.getUrlSeed().getUrl())) {
                return;
            }
            ...
            try {
                String title = htmldoc.select(".arti_title").first().text();
                String text = htmldoc.select(".entry").first().text();
    ...
    static Saver mySaver = new Saver() {
        ...
        public void save(Page page) {
            ...
            if (page.getItems().size() != 0) {
            	...
            	System.out.println("nothing in page");
            }
            return;
        }
    };
